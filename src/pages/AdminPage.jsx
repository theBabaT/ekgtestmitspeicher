...
const AdminPage = () => {
  ...
  const uploadImage = async (file) => {
    ...
    if (error) { alert('Upload-Fehler'); return null; }
  ...
  const handleAddQuestion = async () => {
    ...
    setQuestion({ image: '', text: '', choices: ['', '', '', '', ''], correct: 0, explanation: '' });
  };
  ...
  if (!loggedIn) {
    return (
      <div>
        ...
        <button onClick={() => setLoggedIn(password === 'admin123')}>Login</button>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <input type="text" placeholder="Neues Set" onKeyDown={(e) => {
        if (e.key === 'Enter') { addSet(e.target.value); setSelectedSet(e.target.value); e.target.value = ''; }
      }} />
      <ul>
        {sets.map((set, i) => (
          <li key={i}><button onClick={() => setSelectedSet(set)}>{set} {selectedSet === set ? '✅' : ''}</button></li>
        ))}
      </ul>
      <h3>Neue Frage: {selectedSet}</h3>
      <input type="file" onChange={handleImageUpload} />
      <input type="text" placeholder="Fragetext" value={question.text}
        onChange={(e) => setQuestion({ ...question, text: e.target.value })} />
      {question.choices.map((c, i) => (
        <input key={i} type="text" placeholder={`Antwort ${i + 1}`} value={c}
          onChange={(e) => {
            const choices = [...question.choices]; choices[i] = e.target.value;
            setQuestion({ ...question, choices });
          }} />
      ))}
      <input type="number" min="0" max="4" value={question.correct}
        onChange={(e) => setQuestion({ ...question, correct: Number(e.target.value) })} />
      <textarea placeholder="Erklärung" value={question.explanation}
        onChange={(e) => setQuestion({ ...question, explanation: e.target.value })}></textarea>
